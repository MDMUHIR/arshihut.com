...
<template>
  <div class="main pb-12 min-h-svh bg-stone-100 dark:bg-[#212121] overflow-y-auto">
    ...
    <div class="rounded-lg sm:mt-8 sm:mx-8">
      <!-- <pre>{{ data.orders }}</pre> -->
      <template v-for="(order, index) in data.orders" :key="index">
        <div class="rounded-lg border md:w-full mb-6 bg-white p-6 shadow-md">
          <div class="top-text flex justify-between mb-4">
            ...
            <p
              class="font-bold bg-stone-200 py-2 px-4 rounded-2xl border block"
            >
              Order id: {{ order.id }}
            </p>
            <p
